...
let ftp = {
    ...
    start: function (id) {
        ...
        let item = id ? ftp.item(id) : ftp.next();
        if (item) { ftp.active = true; ftp.send_slice(item); }
    },
    stop: function (id) {
        let item = ftp.item(id);
        let index = ftp.queue.indexOf(item);
...
$file.progress = function onprogress(offset,total) {
    let x = qi('ftp_status'); if (x) x.innerHTML = offset;
};

$file.do = function (rsp) {
    let total = rsp.v[5].v, offset = rsp.v[6].v, block = rsp.v[7].v, status = utf8_arr(rsp.v[9].v);
    switch (status) {
        case 'init':
            if(block == 1) return;
