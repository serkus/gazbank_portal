...
function en_104(o) {
  let l = o.v.length, r = [];
  for (let i = 0; i < l; i++)r[i] = ein(o.v[i]);
  return [104, l, r];
}
function unilen(o) {
...
function en_108(o) {
  let l = o.v.length, r = [];
  for (let i = 0; i < l; i++)r.push(ein(o.v[i]));
  return o.v.length == 0 ? [106] :
         [108, l >>> 24, (l >>> 16) & 255, (l >>> 8) & 255, l & 255, r, 106];
}
function en_116(o) {
  let l=o.v.length,x=[],r = [];
  for (let i = 0; i < l; i++) r.push([ein(o.v[i].k),ein(o.v[i].v)]);
  x = [116, l >>> 24, (l >>> 16) & 255, (l >>> 8) & 255, l & 255];
  return o.v.length == 0 ? x : [x,r];
}
function en_110(o) {
  let s=int_to_bytes(o.v); return [110,s.length,(o.v<0)?1:0].concat(s);
}
...
function big(b) {
  let sk = b == 1 ? sx.getUint8(ix++) : sx.getInt32((a = ix, ix += 4, a));
  let ret = 0, sig = sx.getUint8(ix++), count = sk;
...
function str(b) {
  let dv, sz = (b==2?sx.getUint16(ix):(b==1?sx.getUint8(ix):sx.getUint32(ix)));
  ix += b; let r = sx.buffer.slice(ix, ix += sz);
  return utf8_arr(r);
}
function run(b) {
  let sz = (b == 1 ? sx.getUint8(ix) : sx.getUint32(ix)), r = []; ix += b;
  for (let i = 0; i < sz; i++) r.push(din());
  return r;
}
function rut(b) {
  let sz = (b == 1 ? sx.getUint8(ix) : sx.getUint32(ix)), r = []; ix += b;
  for (let i = 0; i < sz; i++) r.push(din()); din();
  return r;
}
function dic(b) {
  let sz = sx.getUint32(ix), r = []; ix += 4;
  for (let i = 0; i < sz; i++) r.push({k:din(),v:din()});
  return r;
}
function iee(x) {
...
function arr(b) {
  let dv, sz = sx.getUint16(ix); ix += b;
  return new Uint8Array(sx.buffer.slice(ix, ix += sz));
}
function din() {
  let c = sx.getUint8(ix++), x; switch (c) {
...
function int_to_bytes(Int) {
  if (Int % 1 !== 0) return [0];
  let isNegative, OriginalInt, i, Rem, s = [];
  isNegative = (Int < 0);
