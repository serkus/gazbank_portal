...
let ws = { send: function (payload, qos) {
...
function client()      { let c = localStorage.getItem("client"), a;
...
function rnd()         { return Math.floor((Math.random() * nodes)+1); }
  ...
  mqtt.onMessageArrived = function (m) {
        let BERT = m.payloadBytes.buffer.slice(m.payloadBytes.byteOffset,
